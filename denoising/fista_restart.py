...
def fista_rada(L, x0, p, q, proxR, gradF, eps_m=20, eps=None, reset_t=False, max_iter=500):
    """
    ...
    for k in range(1, max_iter+1):
        ...
        yk = xk + ak*(xk - xk_prev)
        xk_prev = np.copy(xk)        
        xk = proxR(gamma, yk - gamma*gradF(yk)).reshape(nx, ny)

        yield xk, xk_prev

        # Restarting with automatic choice of eps(ak)
        if ((yk - xk).T @ (xk - xk_prev) >= 1e-12).all():
            n_restarts += 1
            print("Restart!")
...
def fista_greedy(L, gamma, x0, S, eps, proxR, gradF, max_iter=500):
    """
