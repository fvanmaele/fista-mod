...
and other perturbations. These random distortitions make it difficult
to perform any required picture processing. For example, in the image
deblurring project \cite{Vanmaele-2021} we showed that even a small
amount of noise can lead to poor deblurring results. 

Consequently, traditional methods in image processing attempt to reduce/remove
the noise component prior to further operations. In this project,
we formulate image denoising as a 2-dimensional convex optimization
problem, using a data term and \emph{total variation} as regularizer.
We solve this problem numerically by using FISTA schemes, including
recent modifications with fast convergence by Liang and Schoenlieb
\cite{Liang-2022}. We then test both the convergence of the schemes
and quality of the denoised dimages.
\end{abstract}
...
Let the observed intensity function $f(x,y):[0,1]^{2}\rightarrow\mathbb{R}$
...
$f$. To this end, we consider minimization problems on a Hilbert
space $\mathcal{H}$,
\begin{equation}
...
variation} (TV). Following \cite[1 TV Regularization]{Getreuer-2012},
we define the bounded and total variation in section \ref{subsec:Denoising-problem}.
...
as a solution to the minimization problem{\small{}
\begin{equation}
...
to as the ROF problem. \cite{Rudin-1992} Denoising is performed as
an infinite-dimensional minimization problem, where the search space
is all bounded variation (BV) images. 
...
is smooth, $Du(x)=\nabla u(x)\,\text{d}x$. The \emph{total variation
}$(TV)$ seminorm of $u$ is defined as
\begin{align*}
...
\]
TV is proper, convex and lsc, so we can use it as a regularization
term. \cite[Theorem 3.2]{Petra-2021} This regularizer discourages
the solution from having oscillations, yet is does allow the solution
to have discontinuities. The data term $\int_{\Omega}(f-u)^{2}$ encourages
the solution to be close to the observed image $f$. By this combination,
a minimizer finds a denoised image. If $f\in L^{2}$, the minimizer
of the ROF problem exists, is unique and is stable in $L^{2}$ with
respect to perturbations in $f$.
\begin{rem*}
...
by the \texttt{pyproximal} Python library. For further discussion
on possible discretizations of TV, see \cite[2 Algorithms]{Getreuer-2012}.

The discrete formulation of problem (\ref{eq:ROF}) with an $L_{2}$
data term is then
\begin{equation}
...
$R:=\lambda TV_{I}(u)$ is as mentioned above proper, closed and convex,
and the sum (\ref{eq:ROF-discrete}) has a non-empty set of minimizers.
We now look at concrete methods for solving this problem numerically,
following \cite[1 Introduction]{Liang-2022}.
...
\]
the \emph{proximity operator} of $R$. A closed form of the proximity
operator is in general not available. For the total variation operator,
an approximation can be computed iteratively \cite{Beck-2009}.
...
with the updating rule (\ref{eq:FISTA-CD}), Liang and Schoenlieb
\cite{Liang-2022} propose a modified FISTA scheme which applies the
following rule:
\[
...
\end{itemize}
\begin{figure}[h]
\includegraphics[width=1.15\columnwidth]{images/image0_analysis}

\hfill{}%
\begin{minipage}[c][1\totalheight][t]{0.3\columnwidth}%
\begin{center}
\subfloat[Original]{\begin{centering}
\includegraphics[width=1\columnwidth]{images/image0}
\par\end{centering}
...
\includegraphics[width=1\columnwidth]{images/image0_gaussian}
\par\end{centering}
...
\includegraphics[width=1\columnwidth]{images/image0_gaussian_fista_mod_sigma0\lyxdot 06_tol1\lyxdot 0e-06}
\par\end{centering}
...
\begin{figure}[h]
\includegraphics[width=1.15\columnwidth]{images/image7_analysis}

\hfill{}%
\begin{minipage}[c][1\totalheight][t]{0.3\columnwidth}%
\begin{center}
\subfloat[Original]{\begin{centering}
\includegraphics[width=1\columnwidth]{images/image7}
\par\end{centering}
}
\par\end{center}%
\end{minipage}\hfill{}%
\begin{minipage}[c][1\totalheight][t]{0.3\columnwidth}%
\subfloat[Noisy]{\begin{centering}
\includegraphics[width=1\columnwidth]{images/image7_gaussian}
\par\end{centering}
}%
\end{minipage}\hfill{}%
\begin{minipage}[c][1\totalheight][t]{0.3\columnwidth}%
\subfloat[Denoised]{\begin{centering}
\includegraphics[width=1\columnwidth]{images/image7_gaussian_fista_mod_sigma0\lyxdot 06_tol1\lyxdot 0e-06}
\par\end{centering}
}%
\end{minipage}
...
regardless of lower tolerances achieved for the iterates $\|x^{(k)}-x^{(k-1)}\|$.
FISTA showed slight oscillation of PSNR for the denoised images, while
FISTA-Mod and FISTA-CD had monotonically increasing PSNR for every
tested sample. It should be noted that computing the PSNR depends
on the original images. When these are not available, the iterates
and objective values in the FISTA schemes remain available metrics.
